	...
	for _, antennas := range antennasByFreq {
		...
		for i, pos := range positions {
			for j := i; j < len(positions); j++ {
				...
				first := position{pos[0] + diffX, pos[1] + diffY}
				second := position{positions[j][0] - diffX, positions[j][1] - diffY}
				antinodes += field.addAntinode(first)
				antinodes += field.addAntinode(second)
			}
		}
	}
